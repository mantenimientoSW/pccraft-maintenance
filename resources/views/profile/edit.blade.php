                ...
                @foreach($direcciones as $direccion)
                    <div class="mb-4 p-4 border border-gray-300 rounded-md {{ $direccion->is_default ? 'bg-green-100' : 'bg-white' }}">
                        ...
                        <form action="{{ route('profile.setDefaultAddress', $direccion->ID_Direccion) }}" method="POST">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="mt-4 w-full lg:w-auto px-4 py-2 bg-blue-500 text-white rounded-md">
                                Marcar como Predeterminada
                            </button>
                        </form>
                        @else
                        <p class="mt-4 w-full lg:w-auto px-4 py-2 bg-green-500 text-white rounded-md text-center">
